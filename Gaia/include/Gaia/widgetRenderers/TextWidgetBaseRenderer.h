...
void TplTextWidgetRenderer<T>::update(unsigned int elapsedTime)
{
	...
	//if(myWidget->isEnabled() && myWidget->hasFocus() && (tools::closestInt(myElapsedTime * 2 / 1000.f) % 2 == 0) ) //pb with high fps
	//{
	//	//Redraw if caret was displayed (optimized mode)
	...
	//else
	//{
	//	//Redraw if caret was not displayed
	//	if(myMustDisplayCaret)
	//		myWidget->invalidate();
	...
	if(!myMustDisplayCaret && TplWidgetRenderer<T>::myWidget->isEnabled() && TplWidgetRenderer<T>::myWidget->hasFocus() && change )
	{
		//Redraw if caret was displayed (optimized mode)
		this->myWidget->invalidate();

		myMustDisplayCaret = true;
	}
	else if(change && myMustDisplayCaret)
	{
		//Redraw if caret was not displayed
		TplWidgetRenderer<T>::myWidget->invalidate();
